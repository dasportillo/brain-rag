...
//   brain-rag cloud status   # endpoint, org, seats, pending count
...
async function api(conf, path, body, method = 'POST') {
...
async function login() {
  ...
  writeFileSync(CONF, JSON.stringify({ endpoint, apiKey }, null, 2) + '\n', { mode: 0o600 });
...
async function sync({ review = false } = {}) {
  ...
  const db = openDb();
  const rows = pendingMemories(db);
  const items = rows.map(toPushItem);
  ...
  if (!payload.length) { console.log(`✔ nothing to sync${skipped ? ` (${skipped} without embedding skipped)` : ''}`); return; }

  const mark = db.prepare('UPDATE memories SET synced_at = ? WHERE id = ?');
  let pushed = 0, rejected = 0;
  for (let i = 0; i < payload.length; i += 50) {
    const batch = payload.slice(i, i + 50);
    const { results } = await api(conf, '/v1/memories/push', { memories: batch });
    const now = new Date().toISOString();
    results.forEach((r, j) => {
      if (r.action === 'rejected') { rejected++; console.error(`  ✗ rejected: ${batch[j].title} — ${r.error}`); }
...
export async function main() {
  ...
  switch (action) {
    ...
    default:
      console.log('usage: brain-rag cloud <login|sync|status|logout>  (sync --review = dry run)');
